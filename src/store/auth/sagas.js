...
import { REQUEST_FACEBOOK_LOGIN, REQUEST_FACEBOOK_LOGIN_FAILURE, REQUEST_PHONE_NUMBER_LOGIN, REQUEST_PHONE_NUMBER_LOGIN_FAILURE } from "../../store/auth/types";
...
export function* loginWithPhoneNumber(action) {
  try {
    ...
    handlers.showToast(strings("login.error_retry_login"), "danger");
    yield put({ type: REQUEST_PHONE_NUMBER_LOGIN_FAILURE, message: response.message });
  }
}
